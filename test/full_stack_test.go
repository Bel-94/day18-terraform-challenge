...
func TestFullStackEndToEnd(t *testing.T) {
	...
	// This ties all resources together so you can identify
	// everything belonging to this specific test run.
	uniqueID := random.UniqueId()

	// -------------------------------------------------------
	// Step 1 — Deploy the webserver cluster
	// In a full stack test this would first deploy VPC,
	// then pass VPC outputs into the app module.
	// Here we use the default VPC to keep the test
	// self-contained without a separate VPC module.
	// -------------------------------------------------------
	...
	// In a multi-module test, register defers in REVERSE
	// dependency order so the app is destroyed before the
	// VPC it depends on.
	...
	// It proves the complete path works:
	// Your machine → Internet → ALB → Target Group
	// → EC2 Instance → Web Server → Response
	// -------------------------------------------------------
